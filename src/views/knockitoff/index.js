...
export function render() {
  ...
  wrap.innerHTML = `
    ...
    <section class="kio-start stack" id="kio-start">
      <div class="kio-controls">
        ...
        <div class="kio-buttons"><button id="kio-new" class="button">New Game</button></div>
        <div class="kio-buttons kio-how"><button id="kio-how" class="button button-secondary">How To Play</button></div>
      </div>
    </section>
    ...
    <section class="stack kio-setup hidden" id="kio-setup">
      ...
      <div class="kio-buttons"><button class="button button-secondary" id="kio-back">Back</button></div>
    </section>
    ...
    <section class="stack kio-rules hidden" id="kio-rules">
      ...
      <div class="kio-buttons"><button id="kio-rules-back" class="button button-secondary">Back</button></div>
    </section>
  `;
  ...
  function createCheckerIcon(kind, color, size = 18) {
    ...
    if (kind === 'smiley' || kind === 'frowny') {
      ...
      svg.style.width = `${Math.max(12, size - 6)}px`;
      svg.style.height = `${Math.max(12, size - 6)}px`;
      const dark = '#111827';
      const yellow = '#fde047';
      ...
      faceBg.setAttribute('stroke', dark); faceBg.setAttribute('stroke-width', '1.5');
      ...
      if (kind === 'smiley') {
        ...
        const mouth = document.createElementNS('http://www.w3.org/2000/svg', 'path'); mouth.setAttribute('d', 'M7.2 14.2c1.9 2.2 3.8 3 4.8 3s2.9-.8 4.8-3'); mouth.setAttribute('fill','none'); mouth.setAttribute('stroke', dark); mouth.setAttribute('stroke-width', '2.2'); mouth.setAttribute('stroke-linecap','round');
        svg.append(eyeL, eyeR, mouth);
      } else {
        const lx1 = document.createElementNS('http://www.w3.org/2000/svg', 'path'); lx1.setAttribute('d','M7.4 9.2l2.4 2.4'); lx1.setAttribute('stroke', dark); lx1.setAttribute('stroke-width','2.2'); lx1.setAttribute('stroke-linecap','round');
        const lx2 = document.createElementNS('http://www.w3.org/2000/svg', 'path'); lx2.setAttribute('d','M9.8 9.2l-2.4 2.4'); lx2.setAttribute('stroke', dark); lx2.setAttribute('stroke-width','2.2'); lx2.setAttribute('stroke-linecap','round');
        const rx1 = document.createElementNS('http://www.w3.org/2000/svg', 'path'); rx1.setAttribute('d','M14.0 9.2l2.4 2.4'); rx1.setAttribute('stroke', dark); rx1.setAttribute('stroke-width','2.2'); rx1.setAttribute('stroke-linecap','round');
        const rx2 = document.createElementNS('http://www.w3.org/2000/svg', 'path'); rx2.setAttribute('d','M16.4 9.2l-2.4 2.4'); rx2.setAttribute('stroke', dark); rx2.setAttribute('stroke-width','2.2'); rx2.setAttribute('stroke-linecap','round');
        const mouth = document.createElementNS('http://www.w3.org/2000/svg', 'path'); mouth.setAttribute('d','M7.2 17.5c1.9-2.2 3.8-3 4.8-3s2.9.8 4.8 3'); mouth.setAttribute('fill','none'); mouth.setAttribute('stroke', dark); mouth.setAttribute('stroke-width','2.2'); mouth.setAttribute('stroke-linecap','round');
  ...
  function showWinModal(winner) {
    // Ensure no preview or markers remain
    isPreview = false; clearDirMarkers(); removeMoveArrow();
    let modal = document.getElementById('kio-win');
    ...
    // Solid black body with white text
    box.style.background = '#000000';
    box.style.color = '#ffffff';
    // Two-line layout: "Game Over" then "[Color] Wins"
    const titleTop = document.createElement('div');
    titleTop.style.fontSize = '22px';
    titleTop.style.fontWeight = '800';
    titleTop.textContent = 'Game Over';
    ...
    box.append(titleTop, titleBottom, hint);
    modal.append(box);
    ...
    boardEl.querySelectorAll('.kio-cell').forEach((sqEl, i) => {
      ...
      chip.addEventListener('dragstart', (e)=>{
        ...
        DIRS.forEach(d => {
          ...
          marker.style.border='2px dashed #3f48cc';
    ...
    boardEl.querySelectorAll('.kio-cell').forEach((sqEl, i) => {
      ...
      const showMarkers = (mode='hover') => {
        ...
        DIRS.forEach(d => {
          ...
          marker.style.position='absolute'; marker.style.left='0'; marker.style.top='0'; marker.style.width='100%'; marker.style.height='100%'; marker.style.border='2px dashed #3f48cc'; marker.style.background='rgba(63,72,204,0.12)'; marker.style.borderRadius='8px'; marker.style.zIndex='8'; marker.style.cursor='pointer';
  ...
  function renderRacks(state) {
    ...
    racksEl.innerHTML = '';
    const human = state.phase === 'setup' ? getActiveSetupHuman() : state.players.find(p=>p.type==='human');
    ...
    const faceIcon = (kind) => {
      ...
      if (kind === 'smiley') {
        ...
        eyeL.setAttribute('cx', '8.5'); eyeL.setAttribute('cy', '10'); eyeL.setAttribute('r', '1.4'); eyeL.setAttribute('fill', dark);
        const eyeR = document.createElementNS('http://www.w3.org/2000/svg', 'circle');
        eyeR.setAttribute('cx', '15.5'); eyeR.setAttribute('cy', '10'); eyeR.setAttribute('r', '1.4'); eyeR.setAttribute('fill', dark);
        const mouth = document.createElementNS('http://www.w3.org/2000/svg', 'path');
        // smooth smile curve
        mouth.setAttribute('d', 'M7.2 14.2c1.9 2.2 3.8 3 4.8 3s2.9-.8 4.8-3');
        mouth.setAttribute('fill', 'none'); mouth.setAttribute('stroke', dark); mouth.setAttribute('stroke-width', '2.6'); mouth.setAttribute('stroke-linecap', 'round');
        svg.append(eyeL, eyeR, mouth);
      } else if (kind === 'frowny') {
        const lx1 = document.createElementNS('http://www.w3.org/2000/svg', 'path');
        lx1.setAttribute('d', 'M7.4 9.2l2.4 2.4'); lx1.setAttribute('stroke', dark); lx1.setAttribute('stroke-width', '2.2'); lx1.setAttribute('stroke-linecap', 'round');
        const lx2 = document.createElementNS('http://www.w3.org/2000/svg', 'path');
        lx2.setAttribute('d', 'M9.8 9.2l-2.4 2.4'); lx2.setAttribute('stroke', dark); lx2.setAttribute('stroke-width', '2.2'); lx2.setAttribute('stroke-linecap', 'round');
        const rx1 = document.createElementNS('http://www.w3.org/2000/svg', 'path');
        rx1.setAttribute('d', 'M14.0 9.2l2.4 2.4'); rx1.setAttribute('stroke', dark); rx1.setAttribute('stroke-width', '2.2'); rx1.setAttribute('stroke-linecap', 'round');
        const rx2 = document.createElementNS('http://www.w3.org/2000/svg', 'path');
        rx2.setAttribute('d', 'M16.4 9.2l-2.4 2.4'); rx2.setAttribute('stroke', dark); rx2.setAttribute('stroke-width', '2.2'); rx2.setAttribute('stroke-linecap', 'round');
        const mouth = document.createElementNS('http://www.w3.org/2000/svg', 'path');
        mouth.setAttribute('d', 'M7.2 17.5c1.9-2.2 3.8-3 4.8-3s2.9.8 4.8 3');
        mouth.setAttribute('fill', 'none'); mouth.setAttribute('stroke', dark); mouth.setAttribute('stroke-width', '2.6'); mouth.setAttribute('stroke-linecap', 'round');
        svg.append(lx1, lx2, rx1, rx2, mouth);
      }
      return svg;
  ...
  btnNew.addEventListener('click', () => {
    ...
    draw(currentMask);
    // Assign colors randomly but keep turn order b→r→g→u
    const POL = ['b','r','g','u'];
    const allowed = (pc===2) ? ['b','r'] : (pc===3) ? ['b','r','g'] : POL.slice();
    const turnColors = allowed.slice(); // fixed order set by rules
    // Randomly assign the selected players to these colors by shuffling the chosen list
    ...
    console.log('Knock It Off — initial game state', st);
    // stash on the element for quick access during development
    wrap.__kioState = st;
    gameState = st;
    gameState.phase = 'setup';
    // Setup sequence for local humans in turn order
    const humanColors = gameState.players.filter(p=>p.type==='human').map(p=>p.color);
    gameState.setup = { humanColors, index: 0 };
    renderRacks(gameState);
    // Plan AI placements in state and reveal them gradually in UI
    planAIPlacements();
    revealAIPlacements();
  });
